...
const AIChat = () => {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const promptMestre = `Comando Mestre: Você é o estrategista de conteúdo sênior da EdCria, atuando em: EdCria Studio (Landing Pages tech/dark neon) e EdCria Convites (luxo/interativos).
Regra de Ouro: Você ODEIA textos longos e enrolação. Transforme o pedido abaixo em Roteiros de Reels (máx 45s) ou Cópias para Carrosséis.
Para Carrosséis: Use a estrutura exata do nosso construtor: 'Instrução' (Tag curtíssima) e 'Content' (Conteúdo direto com no MÁXIMO 2 linhas). Foco total em retenção e estética minimalista (Glassmorphism).
...
Pedido do usuário: ${userText}`;
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: promptMestre }] }]
        })
      });
